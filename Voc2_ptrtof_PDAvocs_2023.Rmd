...
#save(prod, file = "prod.RData")
#save(cons, file = "cons.RData")
...
mean.topda.vocs[mean.vocs < 0.2] <- 0 

for(i in 1:ncol(mean.topda.vocs)){
  mean.topda.vocs[,i] <- mean.topda.vocs[,i]/voc.ls$PDA_no[,4]
}
...
trt.color[seq(3,ncol(mean.topda.vocs.bi), by = 2)] <- "#ffffb3" #pure cultures are yellow
trt.color[seq(2,ncol(mean.topda.vocs.bi), by = 2)] <- "#fdb462" #cocultures are orange
...
mean.prod <- as.matrix(mean.prod[rowSums(mean.prod) > 0,])
png("PTR_heatmap_producedvocs_strains_trt_portrait.png", width = 350, height = 600)
heatmap.2(mean.prod, scale = "none", col = c("white", "#fb8072", "#67000d"), trace = "none", hclustfun = function(x) hclust(x, method="ward.D"), margins = c(12,8), ColSideColors = trt.color)
dev.off()
pdf("PTR_heatmap_producedvocs_strains_trt_portrait.pdf")
heatmap.2(mean.prod, scale = "none", col = c("white", "#fb8072", "#67000d"), trace = "none", hclustfun = function(x) hclust(x, method="ward.D"), cexRow = 0.1 + 1/log10(nrow(mean.prod)), margins = c(12,8), ColSideColors = trt.color)
dev.off()
...
#mean.vocs.cons <- mean.vocs[which(rownames(mean.vocs) %in% cons),]
...
dev.off()
pdf("PTR_heatmap_consumedvocs_strains_trt_portrait.pdf")
heatmap.2(as.matrix(mean.cons), scale = "none", col = c("#4c6a7e", "#b2d0e4",	"white", "#fb8072", "#67000d"), trace = "none", hclustfun = function(x) hclust(x, method="ward.D"), margins = c(12,8), ColSideColors = trt.color)
dev.off()

png("heatmap_legend_trt.png")
plot.new()
legend("topleft", legend = c(make.italic("P. helmanticensis"), "Fungal pure culture", "Co-culture", "Insignificant", "Produced", "Consumed"), fill = c("black", "#ffffb3", "#fdb462", "white", "#fb8072", "#80b1d3"), cex = 2, bty = "n")
dev.off()
png("heatmap_legend_esize.png")
plot.new()
legend("topleft", legend = c("Strongly produced", "Produced", "Insignificant", "Consumed", "Strongly consumed"), fill = c("#67000d", "#fb8072",	"white", "#b2d0e4", "#4c6a7e"), cex = 2, bty = "n")
dev.off()
